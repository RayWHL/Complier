...
void ext_var_list(struct node *T){//处理变量列表
    ...
    switch (T->kind){
        ...
        case ARRAY_DEF :    //THERE
            ...
            //T->ptr[1]->offset=T->offset+T->width; //外部变量的偏移量向下传递
            //T->ptr[1]->width=T->width;
            rtn=fillSymbolTable(T->ptr[0]->type_id,newAlias(),LEV,T->type,'V',T->offset);  //最后一个变量名
            if (rtn==-1)
                semantic_error(T->pos,T->type_id, "变量重复定义");
            else T->place=rtn;
...
int match_param(int i,struct node *T){
    ...
    for (j=0;j<num;j++) {
        if (!T){
            semantic_error(pre->pos,"", "函数调用参数太少");
            return 0;
        }
    ...
    if (T){
        switch (T->kind) { 
            ...
            case DIV:   
                ...
                {
                    semantic_error(T->ptr[0]->pos,"", "除数不能为0");
                    break;
                }   
            ...
            case INC:   //a++为右值 if新增
                ...
                {
                    semantic_error(T->ptr[0]->pos,T->ptr[0]->type_id, "右值不能++");
                    break;
                }
...
void semantic_Analysis0(struct node *T) {
    ...
    fillSymbolTable("x","",1,INT,'P',12);//参数列表
